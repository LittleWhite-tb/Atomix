...
void BoardTileSetter::setTile(Board& board, const UVec2& position, int tileID)
{
    assert(position.y < board.m_tiles.size());
    assert(position.x < board.m_tiles[0].size());
    
    board.m_tiles[position.y][position.x] = Tile(tileID);
}
